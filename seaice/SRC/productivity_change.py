...
data_directory = '../DATA/'
obs_sie = xr.open_dataset(data_directory+'icec.mnmean.nc')
...
for j in range(len(months)):
    ...
    x,y = np.meshgrid(obs_sie.lon.values, obs_sie.lat.values)
    ...
    for i in range(0,4):
        yr = str(1981+i*10)
        yr2 = str(1990+i*10)
        data = obs_sie.sel(time=slice(yr+"-"+str("{:02d}".format(month)),yr2+"-"+str("{:02d}".format(month))))
        data_month = data.groupby('time.month').mean('time')
        rgba = cmap(i/3)
        contour = plt.contour(x, y, data_month.icec.sel(month=month), levels=[15], linestyles=ls[i], 
                              colors='#14165c', linewidths=0.5, transform=ccrs.PlateCarree())
        if yr=='1981':
...
cbar.set_label(r'NPP [g C m$^{-2}$ day$^{-1}$] / yr', fontsize=16)
